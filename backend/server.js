...
// ── Trust proxy (required for Render/Nginx) ────
app.set('trust proxy', 1);
...
// ════════════════════════════════════════════════════════════════
// RATE LIMITERS (The Safety Essentials)
// ════════════════════════════════════════════════════════════════
...
const strictLimiter = rateLimit({
  windowMs: 15 * 60 * 1000,
  max: 10,
  message: { success: false, message: 'High frequency requests blocked.' },
});

app.use('/api/', globalLimiter);
app.use('/api/contact', strictLimiter);
app.use('/api/admissions', strictLimiter);
app.use('/api/donate', rateLimit({ windowMs: 15 * 60 * 1000, max: 20 }));

// ════════════════════════════════════════════════════════════════
// API ROUTES (Order Fix: Placed before Static Files)
// ════════════════════════════════════════════════════════════════
const formsRouter  = require('./routes/forms');
const donateRouter = require('./routes/donate');
const adminRouter  = require('./routes/admin');
...
// Catch-all for Frontend SPA (only if not an API path)
app.get('*', (req, res) => {
...
const server = app.listen(PORT, '0.0.0.0', () => {
    ...
    logger.info('🏫 Cornerstone Schools API live');
});
